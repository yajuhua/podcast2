...
public interface SubMapper {
    ...
    @Insert("INSERT INTO sub(uuid, equal, title, link, status, description, image, create_time, check_time, update_time, type," +
            ...
            " cron_expression, xml_conf_name, unix_cron_expression) VALUES(#{uuid}, #{equal}, #{title}, #{link}, " +
            ...
            "#{scheduleType}, #{cronExpression}, #{xmlConfName}, #{unixCronExpression})")
    void addSub(Sub sub);
    ...
    @Update("update sub set type = #{type}, survival_time = #{survivalTime}, cron = #{cron}, is_update = #{isUpdate}," +
            ...
            "xml_conf_name = #{xmlConfName}, unix_cron_expression = {unixCronExpression}  where uuid = #{uuid}")
